...
#### Step H.3 - Update the imgtool version in the board makefile
...
#### Step H.4 - Build the upgrade image
...
The file looks like this — edit `major`, `minor`, `build`, and `name` to match the version set in Step H.3:
...
#### Step H.6 - Wrap the image with the CSMP TPD header
...
- **Argument 1** — path to the `.bin` built in Step H.4
...
#### Step H.7 - Transfer the image to the FND server
...
#### Step H.8 - Upload the firmware image to FND
...
#### Step H.9 - Assign the image to a device group and start upload
...
4. In the dialog:
   ...
   - **Select an Image:** choose the image you uploaded in Step H.7 (`ota_upgrade_v2.bin`)
5. Click **Upload Image** to start sending blocks to the device.

#### Step H.10 - Monitor the OTA progress
...
#### Step H.11 - Reboot the device to apply the new firmware
...
2. Check the **Firmware** tab — the running firmware version should now show `2.0.0` (or whatever version you set in Step H.3).
...
| H.7 | Copy to FND host | `scp <out.bin> user@fnd-server:/tmp/` |
| H.8 | Upload image to FND | CONFIG → Firmware Update → Images → PLC-RF → + |
| H.9 | Assign to group | CONFIG → Firmware Update → Groups → Upload Image |
| H.10 | Monitor | Logs tab on the group firmware page |
| H.11 | Reboot device | Config → Firmware Update → Image List → Actions column → calendar icon |
| H.12 | Verify | Device detail → Firmware tab shows new version |
